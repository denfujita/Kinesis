...
from src.env.myolegs_ball_reach import MyoLegsBallReach

logger = logging.getLogger(__name__)

class AgentBallReach(AgentIM):
    """
    AgentBallReach is a specialized reinforcement learning agent for humanoid environments,
    adapting AgentIM for a ball reaching task, in accordance with the MyoLegsBallReach environment.
    """
    ...
    def setup_env(self):
        self.env = MyoLegsBallReach(self.cfg)
        logger.info("BallReach environment initialized.")

    def eval_policy(self, epoch = 0, dump = False, runs = 100):
        logger.info("Starting policy evaluation on ball reaching.")
