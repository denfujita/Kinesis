...
\title{DEP-Enhanced Exploration for\\Musculoskeletal Motion Imitation}

\author{
  James Liu \quad Jaden Chen \\
  Department of Computer Science, Stanford University \\
  \texttt{\{jihaoliu, jaden1\}@stanford.edu}
}
...
The key limitation of standard PPO in high-dimensional muscle spaces is that diagonal Gaussian exploration $a \sim \mathcal{N}(\mu_\theta(s), \sigma^2 I)$ treats each muscle independently. Because multiple muscles act on each joint, the net torque $\tau_j = \sum_i r_i f_i$ is a sum over muscle forces $f_i$ with moment arms $r_i$; independent random activations tend to cancel, producing negligible net movement and poor gradient signal.
...
We use the Kinesis MoE architecture, where the composite policy is:
\begin{equation}
\pi(a \mid s) = \textstyle\sum_k g_k(s)\,\pi_k(a \mid s)
\end{equation}
Gating weights $g_k(s)$ route states to specialized expert sub-policies $\pi_k$, each trained on a motion subset discovered via negative mining. Each expert is a 6-layer MLP $[2048, 1536, 1024, 1024, 512, 512]$ with SiLU activations. The gate is a 3-layer MLP $[1024, 512, 256]$. Despite MoE's expressiveness, exploration remains the bottleneck in high-dimensional muscle spaces. PPO hyperparameters: $\epsilon_\text{clip}=0.2$, $\gamma=0.99$, $\lambda_\text{GAE}=0.95$, policy LR $= 5 \times 10^{-5}$, value LR $= 3 \times 10^{-4}$.
...
\toprule
\textbf{Mod.} & \textbf{Method} & \textbf{Coord$\uparrow$} & \textbf{Div.$\uparrow$} & \textbf{Disp.(m)$\uparrow$} & \textbf{ActV$\downarrow$} \\
\midrule
\multirow{2}{*}{80}
...
Table~\ref{tab:main} summarizes the motion imitation performance of PPO baseline vs.\ PPO+DEP across three musculoskeletal morphologies trained with AMASS reference motions.
...
\begin{tabular}{llcccc}
\toprule
\textbf{Mod.} & \textbf{Method} & \textbf{MPJPE$\downarrow$} & \textbf{RMSE$\downarrow$} & \textbf{ActV.} & \textbf{Cov.$\uparrow$} \\
& & (mm) & (rad) & & \\
\midrule
\multirow{2}{*}{80}
& PPO & 80.9 & 0.098 & 27.5 & 0.90 \\
& PPO+DEP & \textbf{49.4} & \textbf{0.059} & 43.1 & \textbf{0.93} \\
\midrule
\multirow{2}{*}{86}
& PPO & 82.0 & 0.098 & 27.6 & 0.90 \\
& PPO+DEP & \textbf{50.5} & \textbf{0.061} & 43.8 & \textbf{0.94} \\
\midrule
\multirow{2}{*}{290}
& PPO & 104.8 & 0.126 & 28.5 & 0.82 \\
& PPO+DEP & \textbf{66.5} & \textbf{0.079} & 44.7 & \textbf{0.88} \\
\bottomrule
\end{tabular}
\end{table}
...
We validated the complete pipeline with integration tests (Table~\ref{tab:pipeline}).
...
\midrule
Module imports & PASS & torch 2.10, mujoco 3.6 \\
MuJoCo loading & PASS & 17 bodies, 80 actuators \\
MuJoCo step & PASS & 10 steps, correct state \\
DEP+MuJoCo & PASS & 50 steps, 4 DEP / 46 RL \\
DEP tests (7/7) & PASS & shapes, self-org, switching \\
Full environment & PASS & obs=(469,), nu=80 \\
\bottomrule
\end{tabular}
\end{table}
...
Training was conducted with a single seed per morphology configuration; multi-seed runs would strengthen statistical significance. The SMPL body model and KIT-ML dataset require license agreements, limiting direct reproducibility. Cross-species evaluation requires sourcing animal musculoskeletal models and motion data.
...
\begin{enumerate}[nosep, leftmargin=*]
\item \textbf{Scaling}: Extend to the full Kinesis MoE negative-mining pipeline with larger AMASS motion datasets and multi-seed evaluation.
...
James Liu and Jaden Chen contributed equally. James Liu implemented the DEP exploration module, integrated it with the Kinesis PPO pipeline, designed and ran the MuJoCo physics experiments, and wrote the majority of the report. Jaden Chen implemented the multi-morphology evaluation framework, set up the training infrastructure across the three MyoSuite models, produced the report figures, and contributed to the experimental analysis and writing.

\section*{Source Code}
Source code is available at \url{https://github.com/denfujita/Kinesis}. The repository includes the full training pipeline, DEP integration, evaluation scripts, and configuration files for all experiments.
